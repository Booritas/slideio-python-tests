...
class TestCZI(unittest.TestCase):
    ...
    def test_file_metadata(self):
        ...
        """
        image_path =Tools().getImageFilePath("czi","pJP31mCherry.czi",ImageDir.PUBLIC)
    ...
    def test_file_metadata2(self):
        ...
        """
        image_path =Tools().getImageFilePath("czi","08_18_2018_enc_1001_633.czi",ImageDir.PUBLIC)
    ...
    def test_read_block_2d(self):
        ...
        channel_image_paths = [
            Tools().getTestImagePath("czi","pJP31mCherry_b0t0z0c0x0-512y0-512.bmp"),
            Tools().getTestImagePath("czi","pJP31mCherry_b0t0z0c1x0-512y0-512.bmp"),
            Tools().getTestImagePath("czi","pJP31mCherry_b0t0z0c2x0-512y0-512.bmp")
            ]
        slide = slideio.open_slide(image_path, "CZI")
        self.assertTrue(slide is not None)
        scene = slide.get_scene(0)
    ...
    def test_read_block_4d(self):
        ...
        for channel_index in range(scene.num_channels):
            ...
            for slice_index in range(slices[0], slices[1]):
                ...
                ref_image_name = "pJP31mCherry_b0t0z" + \
                    str(slice_index) + "c" + str(channel_index) + \
                    "x0-512y0-512.bmp"
                ref_image_path = Tools().getTestImagePath("czi",ref_image_name)
                reference_raster = cv.imread(ref_image_path,cv.IMREAD_GRAYSCALE)
    ...
    def test_corrupted_image(self):
        ...
        """
        # Image to test
        image_path =Tools().getImageFilePath("czi","corrupted.czi",ImageDir.PUBLIC)
        with pytest.raises(RuntimeError):
            slideio.open_slide(image_path, "CZI")
